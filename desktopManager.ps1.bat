# 2> nul || @echo off & powershell -WindowStyle hidden -ExecutionPolicy Bypass -C clear;$basename = '%~f0';$files = '%~f1';$argz = '%~1';Invoke-Expression $(Get-Content -Raw %0) & exit
# The above line allows us to escape with a polyglot so this file can rest as
# A batch file allowing file input to be worked on. 
...
# Refreshes a desktop. Directly gets the dynamically loaded library in order to access the code
# that refreshes the desktop on our behalf. 
...
# The function for setting a known folder path.
...
# Get the current location of the desktop. 
$desktopCurrent = get-KnownFolderPath "Desktop"

# If setup option is provided
if ($argz -eq "-setup") { 
    $shell = New-Object -ComObject WScript.Shell;
	$shortcut=$shell.CreateShortcut("$desktopCurrent\Move Desktop To....lnk")
    $shortcut.TargetPath = $basename
    $shortcut.WindowStyle = 7
	$shortcut.WorkingDirectory = "$basedir"
	$shortcut.Description = "Drag and drop a folder on here to change the desktop location to that folder. Double clicking this will change one directory up"
	$shortcut.Save()
    echo "Setup complete. Check your desktop for a shortcut. Place this shortcut in the public desktop. (Requires admin privs)"
    exit;
}

#echo "Passed file = $files";
#echo "Current desktop = $desktopCurrent";
#echo "Current location = $place"


# If a file was passed,
if ($files) {

    # And if the files is a folder,
    if ((Get-Item $files) -is [System.IO.DirectoryInfo])
    {
        # Set paths and refresh
        Set-KnownFolderPath "Desktop" $files;
        Refresh-Explorer;
    }
    else
    {
...
{
    #set-location $desktopCurrent\..
    Set-KnownFolderPath "Desktop" $desktopCurrent\..
    Refresh-Explorer;
    #Set-KnownFolderPath("Desktop", (Get-Location))
}